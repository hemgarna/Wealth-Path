...
import DefenseStrategyForm from "@/components/defense-strategy/defense-strategy-form"
import { createClient as createServiceClient } from "@supabase/supabase-js"

export default async function DefenseStrategyFormPage({
  searchParams,
}: {
  ...
  if (clientId && clientId !== user.id) {
    ...
    if (supabaseUrl && serviceRoleKey) {
      ...
      const result = await serviceClient.from("defense_strategy").select("*").eq("user_id", targetUserId).maybeSingle()

      existingData = result.data
      fetchError = result.error
      console.log("[v0] Using service role client for advisor/CEO view")
    } else {
      const result = await supabase.from("defense_strategy").select("*").eq("user_id", targetUserId).maybeSingle()
  ...
  console.log("[v0] ========== DEFENSE STRATEGY DATA LOADING ==========")
